...
var FundVaultAbi = [
  ...
  },
  {
    ...
    inputs: [
      ...
      {
        name: "compSupplyAmount",
        type: "uint256",
        internalType: "uint256"
      },
      {
        name: "compWithdrawAmount",
        type: "uint256",
        internalType: "uint256"
      }
    ],
  ...
  {
    type: "function",
    name: "setRebalanceRiskThreshold",
    inputs: [
      {
        name: "newThreshold",
        type: "uint8",
...
var getActualBalance = (runtime2, chain) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: chain.chainName,
    isTestnet: true
  ...
  const callData = encodeFunctionData({
    ...
    args: [chain.fundVaultAddress]
  });
  const contractCall = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: chain.mockUSDCAddress,
      data: callData
    }),
    blockNumber: LAST_FINALIZED_BLOCK_NUMBER
  }).result();
...
var getCurrentReserves = (runtime2, chain) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: chain.chainName,
    isTestnet: true
  ...
  const contractCall = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: chain.proofOfReserveOracleAddress,
      data: callData
    }),
    blockNumber: LAST_FINALIZED_BLOCK_NUMBER
  }).result();
...
var updateReserves = (runtime2, chain, totalReserves, reportHash) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: chain.chainName,
    isTestnet: true
  ...
  if (resp.txStatus !== TxStatus.SUCCESS) {
    throw new Error(`Failed to update ProofOfReserveOracle on ${chain.chainName}: ${resp.errorMessage || resp.txStatus}`);
  }
...
var runProofOfReserveWorkflow = async (runtime2) => {
  const logger = new StructuredLogger(runtime2);
  logger.info(`\uD83D\uDE80 Starting Multi-chain Proof of Reserve Verification (${runtime2.config.evms.length} chains)`);
  return withErrorHandling(async () => {
    ...
    for (const chain of runtime2.config.evms) {
      ...
      const actualBalance = getActualBalance(runtime2, chain);
    ...
    const currentReserves = getCurrentReserves(runtime2, primaryChain);
    const diff = aggregatedBalance > currentReserves ? aggregatedBalance - currentReserves : currentReserves - aggregatedBalance;
    if (diff < 1000000n) {
    ...
    const ipfsHash = firebase.uploadReserveReport(runtime2, {
      timestamp: Date.now(),
    ...
    const txHashes = [];
    for (const chain of runtime2.config.evms) {
      const txHash = updateReserves(runtime2, chain, aggregatedBalance, ipfsHash);
  ...
  }, { operation: "Proof of Reserve Verification", runtime: runtime2 });
};
var eventAbi = parseAbi([
  "event ReserveVerificationRequested(address indexed requester, uint256 timestamp)"
]);
var eventSignature = "ReserveVerificationRequested(address,uint256)";
var onLogTrigger = async (runtime2, log) => {
  ...
  runtime2.log(`ReserveVerificationRequested by ${decodedLog.args.requester} at ${decodedLog.args.timestamp}`);
  return runProofOfReserveWorkflow(runtime2);
};
