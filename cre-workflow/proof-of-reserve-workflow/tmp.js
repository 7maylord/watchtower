...
var ProofOfReserveOracleAbi = [
  ...
  {
    type: "function",
    name: "DEFAULT_RESERVE_THRESHOLD",
    inputs: [],
    outputs: [
      {
        name: "",
    ...
    outputs: [
      {
        name: "reserves",
        type: "tuple",
        internalType: "struct IProofOfReserveOracle.ReserveData",
        components: [
          {
            name: "onChainReserves",
            type: "uint256",
            internalType: "uint256"
          },
          {
            name: "custodianReserves",
            type: "uint256",
            internalType: "uint256"
          },
          {
...
var configSchema = exports_external.object({
  ...
  proofOfReserveOracleAddress: exports_external.string(),
  fundVaultAddress: exports_external.string(),
  mockUSDCAddress: exports_external.string(),
  chainSelectorName: exports_external.string(),
  gasLimit: exports_external.string(),
  custodianAPI: exports_external.object({
...
var getActualBalance = (runtime2) => {
  ...
  const callData = encodeFunctionData({
    ...
    args: [runtime2.config.fundVaultAddress]
  });
  const contractCall = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: runtime2.config.mockUSDCAddress,
      data: callData
    }),
    blockNumber: LAST_FINALIZED_BLOCK_NUMBER
  }).result();
...
var getCurrentReserves = (runtime2) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.chainSelectorName,
    isTestnet: true
  ...
  const contractCall = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: runtime2.config.proofOfReserveOracleAddress,
      data: callData
    }),
    blockNumber: LAST_FINALIZED_BLOCK_NUMBER
  }).result();
...
var fetchCustodianReserves = (runtime2) => {
  if (!runtime2.config.custodianAPI.enabled) {
    runtime2.log("\uD83D\uDCDD Custodian API disabled, using on-chain balance only");
    return 0n;
  }
  runtime2.log(`\uD83D\uDCE1 Fetching custodian reserves from: ${runtime2.config.custodianAPI.url}`);
  return 0n;
};
var updateReserves = (runtime2, totalReserves, reportHash) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.chainSelectorName,
    isTestnet: true
  ...
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  runtime2.log(`\uD83D\uDD04 Updating ProofOfReserveOracle with reserves: ${totalReserves}`);
  const callData = encodeFunctionData({
    abi: ProofOfReserveOracleAbi,
  ...
  const resp = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.proofOfReserveOracleAddress,
  ...
  if (txStatus !== TxStatus.SUCCESS) {
    throw new Error(`Failed to update ProofOfReserveOracle: ${resp.errorMessage || txStatus}`);
  }
  const txHash = resp.txHash || new Uint8Array(32);
  runtime2.log(`✅ ProofOfReserveOracle updated! TxHash: ${bytesToHex(txHash)}`);
  return bytesToHex(txHash);
};
var verifyProofOfReserve = (runtime2) => {
  runtime2.log("\uD83D\uDE80 Starting Proof of Reserve Verification...");
  const totalAssets = getTotalAssets(runtime2);
  const assetsInUSDC = Number(totalAssets) / 1e6;
  runtime2.log(`\uD83D\uDCCA Total assets: $${assetsInUSDC.toLocaleString()} USDC`);
  const actualBalance = getActualBalance(runtime2);
  const balanceInUSDC = Number(actualBalance) / 1e6;
  runtime2.log(`\uD83D\uDCB0 Actual USDC balance: $${balanceInUSDC.toLocaleString()}`);
  const custodianReserves = fetchCustodianReserves(runtime2);
  const totalReserves = actualBalance + custodianReserves;
  const reservesInUSDC = Number(totalReserves) / 1e6;
  runtime2.log(`\uD83C\uDFE6 Total reserves: $${reservesInUSDC.toLocaleString()} USDC`);
  let reserveRatio = 1e4;
  if (totalAssets > 0n) {
    reserveRatio = Number(totalReserves * 10000n / totalAssets);
  }
  runtime2.log(`\uD83D\uDCC8 Reserve ratio: ${(reserveRatio / 100).toFixed(2)}%`);
  const currentReserves = getCurrentReserves(runtime2);
  const diff = totalReserves > currentReserves ? totalReserves - currentReserves : currentReserves - totalReserves;
  if (diff < 1000000n) {
    runtime2.log(`⏭️  Reserve change too small, skipping update`);
    return "No update needed";
  }
  const reportHash = `QmPoR${Number(totalReserves)}`;
  const txHash = updateReserves(runtime2, totalReserves, reportHash);
  runtime2.log("✅ Proof of Reserve Verification Complete!");
  return txHash;
};
var onCronTrigger = (runtime2, payload) => {
  runtime2.log(`⏰ Cron triggered - starting PoR verification`);
  return verifyProofOfReserve(runtime2);
};
