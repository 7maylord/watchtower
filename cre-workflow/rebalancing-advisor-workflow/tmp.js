...
var getPortfolioData = (runtime2, chain) => {
  ...
  const assetsResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
  ...
  const sharesResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: chain.fundVaultAddress,
      data: sharesCallData
  ...
  const priceResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: chain.fundVaultAddress,
      data: priceCallData
...
var runRebalancingAdvisoryWorkflow = async (runtime2) => {
  const logger = new StructuredLogger(runtime2);
  logger.info(`\uD83D\uDE80 Starting Multi-chain Rebalancing Advisory Analysis (${runtime2.config.evms.length} chains)`);
  return withErrorHandling(async () => {
    const primaryChain = runtime2.config.evms[0];
    const portfolioData = getPortfolioData(runtime2, primaryChain);
    const assetsInUSDC = Number(portfolioData.totalAssets) / 1e6;
    logger.info("Portfolio data retrieved", {
      totalAssets: `$${assetsInUSDC.toLocaleString()}`,
      chain: primaryChain.chainName
    });
    const riskScore = getCurrentRiskScore(runtime2, primaryChain);
    ...
    let ipfsHash = "N/A";
    try {
      ipfsHash = firebase.uploadRebalancingReport(runtime2, uploadData);
    } catch (uploadError) {
...
var eventSignature = "RebalanceRequested(address,uint256)";
var onLogTrigger = async (runtime2, log) => {
  ...
  runtime2.log(`RebalanceRequested by ${decodedLog.args.requester} at ${decodedLog.args.timestamp}`);
